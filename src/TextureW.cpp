...
bool TextureW::setRenderer(std::shared_ptr< SDL_Renderer > ren)
{
    ...
    {
        Log::log(std::cout, "TextureW::setRenderer()");
        return true;
    }
    else
    {
...
bool TextureW::loadTexture(std::string filePath)
{
    if (!this->isRendererSet())
    {
        Log::log(std::cout, "Renderer not set");
        return false;
    }
    ...
    if (surface == nullptr)
    {
        Log::log(std::cout, "Invalid filePath, unable to load texture");
        return false;
    }
    else
    {
        ...
        {
            Log::log(std::cout, "Unable to create texture from surface");
        }
        else
        {
            this->w = surface->w;
...
bool TextureW::loadTextureFromText(std::string text, SDL_Color color)
{
    SDL_Surface* surface;

    if (this->isFontSet())
    {
        surface = TTF_RenderText_Blended_Wrapped(this->font.get(), text.c_str(), color, 250);
    }
    else
    {
        Log::log(std::cout, "Invalid font file path");
    }

    if (surface == nullptr)
    {
        Log::log(std::cout, "Unable to create surface from text");
        SDL_FreeSurface(surface);
        return false;
    }
    else
    {
        ...
        if (this->texture == nullptr)
        {
            Log::log(std::cout, "Unable to create texture from text");
        }
        else
        {
            this->w = surface->w;
...
bool TextureW::setFont(std::string fontFile, int fontSize)
{
    ...
    {
        Log::log(std::cout, "Unable to load font: ");
        return false;
    }
}
